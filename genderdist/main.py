...
def plot_gender_distribution(df, gender, output_filename):
    ...
    paper_type_handles = [Line2D([0], [0], marker='o', color=color, label=paper_type, linestyle='None')
                          for paper_type, color in paper_type_colors.items()]
    question_handles = [Line2D([0], [0], marker='o', color='black', label='with Question', linestyle='None'),
                        Line2D([0], [0], marker='s', color='black', label='without Question', linestyle='None')]

    # Adjust the figure size if necessary
    fig.set_size_inches(10, 6)

    # Create legends outside the plot area
    paper_type_legend = ax.legend(handles=paper_type_handles, title='Paper Type', loc='upper left', bbox_to_anchor=(1, 1))
    question_legend = ax.legend(handles=question_handles, title='Question Availability', loc='upper left', bbox_to_anchor=(1, 0.7))
    ax.add_artist(paper_type_legend)  # Add the first legend back after adding the second

    # Adjust layout to make room for the legends
    plt.tight_layout(rect=[0, 0, 0.8, 1])  # Adjust the rect parameter as needed
...
plot_gender_distribution(df, 'Male', 'male_participant_distribution2.png')
plot_gender_distribution(df, 'Female', 'female_participant_distribution2.png')
